...
class UserTempResourceIT {
    ...
    void putExistingUserTemp() throws Exception {
        ...
        restUserTempMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedUserTemp.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedUserTemp))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingUserTemp() throws Exception {
        ...
        restUserTempMockMvc
            .perform(
                put(ENTITY_API_URL_ID, userTemp.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(userTemp))
            )
            .andExpect(status().isBadRequest());
        ...
        restUserTempMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(userTemp))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamUserTemp() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restUserTempMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(userTemp)))
    ...
    void patchNonExistingUserTemp() throws Exception {
        ...
        restUserTempMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, userTemp.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(userTemp))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamUserTemp() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restUserTempMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(userTemp)))
            .andExpect(status().isMethodNotAllowed());
