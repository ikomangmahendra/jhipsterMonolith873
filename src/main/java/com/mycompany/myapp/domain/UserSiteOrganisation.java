...
public class UserSiteOrganisation implements Serializable {
    ...
    @OneToMany(fetch = FetchType.LAZY, mappedBy = "userSiteOrganisation")
    @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
    @JsonIgnoreProperties(value = { "user", "userSiteOrganisation" }, allowSetters = true)
    ...
    public void setRoles(Set<Role> roles) {
        if (this.roles != null) {
            this.roles.forEach(i -> i.setUserSiteOrganisation(null));
        }
        if (roles != null) {
            roles.forEach(i -> i.setUserSiteOrganisation(this));
        }
        this.roles = roles;
    }
    ...
    public UserSiteOrganisation addRoles(Role role) {
        this.roles.add(role);
        role.setUserSiteOrganisation(this);
        return this;
    }

    public UserSiteOrganisation removeRoles(Role role) {
        this.roles.remove(role);
        role.setUserSiteOrganisation(null);
        return this;
    }
