...
export type PartialUpdateUserTemp = Partial<IUserTemp> & Pick<IUserTemp, 'id'>;
...
export class UserTempService {
  ...
  update(userTemp: IUserTemp): Observable<EntityResponseType> {
    return this.http.put<IUserTemp>(`${this.resourceUrl}/${this.getUserTempIdentifier(userTemp)}`, userTemp, { observe: 'response' });
  }

  partialUpdate(userTemp: PartialUpdateUserTemp): Observable<EntityResponseType> {
    return this.http.patch<IUserTemp>(`${this.resourceUrl}/${this.getUserTempIdentifier(userTemp)}`, userTemp, { observe: 'response' });
  }
