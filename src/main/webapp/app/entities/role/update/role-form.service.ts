...
type RoleFormDefaults = Pick<NewRole, 'id' | 'userSiteOrganisations'>;

type RoleFormGroupContent = {
  id: FormControl<IRole['id'] | NewRole['id']>;
  user: FormControl<IRole['user']>;
  userSiteOrganisations: FormControl<IRole['userSiteOrganisations']>;
};
...
export class RoleFormService {
  createRoleFormGroup(role: RoleFormGroupInput = { id: null }): RoleFormGroup {
    ...
    return new FormGroup<RoleFormGroupContent>({
      ...
      user: new FormControl(roleRawValue.user),
      userSiteOrganisations: new FormControl(roleRawValue.userSiteOrganisations ?? []),
    });
  }
